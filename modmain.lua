...
local BUFF_LIST = {
                    ...
                    for _, ent in pairs(ents) do
                        if ent.prefab and (
                            ...
                            if fx then
                                ...
                                fx:DoTaskInTime(3, function() fx:Remove() end)
                            end
                        end
                    end
                end
    ...
    },
    {
        ...
        fn = function(player)
            -- 召唤一个跟随玩家的小动物
            local pet_type = {"rabbit", "perd", "penguin", "butterfly"}
                ...
                local follow_task = shadow:DoPeriodicTask(0.5, function()
                    if player:IsValid() and shadow:IsValid() then
                        ...
                        if shadow.components.combat then
                            local target = FindEntity(shadow, 10, function(ent)
                                return ent.components.combat and 
    ...
    inst:WatchWorldState("cycles", function()
        ...
        if currentday > lastday then
            ...
            if currentday <= lastday + 2 then
                ...
                if RANDOM_PLAYERS_COUNT >= 100 then
